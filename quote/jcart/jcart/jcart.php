...
function decode($itemcode) {
$options = explode("-", $itemcode);
$paper = $options[0];
$length = $options[1];
$width = $options[2];
$bleed = $options[3];
...
if ($spiral) {
	echo "Spiral Bound: Yes\n";
}
if ($saddle) {
	echo "Saddle Stitched: Yes\n";
}
if ($round)	{
	echo "Rounded Corners: Yes\n";
}
if ($uvspot) {
	echo "UV Spot: Yes\n";
}
	...
	public function display_cart() {
		...
		if($this->itemCount > 0) {
			...
			foreach($this->get_contents() as $item)	{
				...
				<textarea id="notes" form="order" rows="10" cols="30"  name="notes<?php echo $item['id']; ?>"><?php
				decode($item['id']); ?>
				</textarea><br><br> <?php
		...
		echo tab(4) . "<input type='$inputType' $src name='jcartUpdateCart' id='hideme' value='{$config['text']['update']}' class='jcart-button' />\n";
		...
		echo tab(4) . "<input type='$inputType' $src name='jcartEmpty' id='hideme' value='{$config['text']['emptyButton']}' class='jcart-button' />\n";
		echo tab(3) . "</div>\n";
